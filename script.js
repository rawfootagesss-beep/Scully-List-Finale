...
function calculatePoints(index) {
    const base = 100;
    const pts = base - (index * 0.6);
    return Math.max(pts, 5).toFixed(1);
}
...
function renderLevels(filter = "") {
    ...
    db.forEach((lvl, i) => {
        if (lvl.name.toLowerCase().includes(filter.toLowerCase())) {
            container.innerHTML += `
                <div class="level-card" onclick="openDetails(${i})">
                    <div class="level-thumb"><img src="https://img.youtube.com/vi/${lvl.id}/mqdefault.jpg"></div>
                    <div class="level-info"><h2>${lvl.name}</h2><p style="color:#ff6600; font-weight:bold; font-size:0.7rem;">+${calculatePoints(i)} PTS</p></div>
...
function openDetails(index) {
    ...
    body.innerHTML = `
        <h1 style="color:#ff6600; margin-bottom:0;">${lvl.name}</h1>
        <p style="font-size:0.7rem; color:#444; margin-bottom:15px;">TOP #${index + 1} • ${calculatePoints(index)} PTS</p>
        <iframe width="100%" height="250" src="https://www.youtube.com/embed/${lvl.id}" frameborder="0" allowfullscreen></iframe>
        <div style="text-align:left; margin-top:15px;">
            <p style="margin:0;"><b style="color:#ff6600">FIRST VICTOR:</b> ${lvl.first || 'Nadie'}</p>
            <p style="font-size:0.8rem; color:#888; margin-top:5px;"><b>OTROS:</b> ${victors}</p>
        </div>`;
